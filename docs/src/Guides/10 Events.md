...
- [Error](/interactions.py/API Reference/API Reference/events/internal/#interactions.api.events.internal.Error) triggers whenever an error occurs while the bot is running. By default, this event just logs the errors to the terminal with its traceback. This event by itself does not post an error message to the user, but some of its subtypes do. For *tracking* these errors outside the terminal, check our [error tracking guide](../25 Error Tracking).
- [CommandError](/interactions.py/API Reference/API Reference/events/internal/#interactions.api.events.internal.CommandError), [ComponentError](/interactions.py/API Reference/API Reference/events/internal/#interactions.api.events.internal.ComponentError), and [ModalError](/interactions.py/API Reference/API Reference/events/internal/#interactions.api.events.internal.ModalError) are error events that can send error messages to the user (its traceback by default). Add a listener to these events (preferably with `disable_default_listeners=True`) to create and send a custom error message (or not send them at all).
