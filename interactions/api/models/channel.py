...
class Channel(DictSerializerMixin):
    ...
    def __init__(self, **kwargs):
        ...
        self.permission_overwrites = (
            [Overwrite(**overwrite) if isinstance(overwrite, dict) else overwrite for overwrite in self._json.get("permission_overwrites")]
    ...
    async def modify(
        ...
        _parent_id = (int(self.parent_id) if self.parent_id else None) if parent_id is MISSING else int(parent_id)
