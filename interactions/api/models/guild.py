...
class Guild(DictSerializerMixin):
    ...
    def __init__(self, **kwargs):
        ...
        if (
            not self.members
            and self._client
            and len(self._client.cache.guilds.view) > 1
            and self._client.cache.guilds.values[str(self.id)].members
        ):
            members = self._client.cache.guilds.values[str(self.id)].members
