...
if sys.version_info < (3, 11):
    __all__ = ("LibraryException", "_parse_error")
else:
    __all__ = ("LibraryException", "LibraryExceptions", "_parse_error")

log = getLogger(__name__)


def _parse_error(_data: dict) -> List[tuple]:
    """
...
if sys.version_info < (3, 11):
    ...
    class LibraryException(Exception):
        ...
        __slots__ = {"code", "severity", "message", "data", "original_payload"}
        ...
        def lookup(code: int) -> str:
            return error_lookup.get(code, f"Unknown error: {code}")
        ...
        def __init__(self, code: int = 0, message: str = None, severity: int = 0, **kwargs):
            ...
            if (
                ...
                _fmt_error: List[tuple] = _parse_error(self.data["errors"])
            ...
            if _fmt_error:
                ...
                if _flag:
                    ...
                    text = f"  {self.message}{'' if self.message.endswith('.') else '.'}\n"
    ...
    LibraryExceptions = type("LibraryExceptions", (object,), {})
    # idk this is just to prevent stupid user errors, dk if we really need it?
    ...
    class LibraryException(Exception):
        ...
        def __init__(self, code: int = 0, message: str = None, severity: int = 0, **kwargs):
            ...
            # fmt should not be used here because this is intended to be used with an exc group if needed

            _flag: bool = (
                self.message.lower() in self.lookup(self.code).lower()
            )  # creativity is hard

            spaces = "  "  # formatting fine in python shell, just bad in pycharm console(?)

            if _flag:
                text = ""
            else:
                text = f"{spaces}{self.message}{'' if self.message.endswith('.') else '.'}\n"

            super().__init__(
                "\n"
                f"{spaces}Error {self.code} | {self.lookup(self.code)}\n"
                f"{text}"
                f"{spaces}Severity {self.severity}."
            )

    class LibraryExceptions(ExceptionGroup):  # noqa todo remove after flake8 knows it
        """
        A class object representing multiple exceptions occurring at the same time.

        Limited to 3.11+
        """

        def __new__(cls, *exc: LibraryException):

            frame = inspect.currentframe()
            _frame = frame.f_back

            # inconsistent tb in 3.11 rc2 - todo test again after full release

            while "interactions" in str(_frame):
                _frame = _frame.f_back

            data = [elem for elem in str(_frame).split("\\") if elem]

            return super().__new__(
                cls,
                "\n"
                f"   Exception origins from function call on line {_frame.f_lineno} in ..\\{data[-2]}\\{data[-1].split('.py')[0]}.py\n"
                "   Multiple errors occurred during your request:",
                list(exc),
            )
